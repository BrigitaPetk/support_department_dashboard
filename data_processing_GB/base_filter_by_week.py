...
class FilterFinalData_GB():
    def final_data(self, filter_direktorija_GB, final_direktorija_GB):

        df = pd.read_csv(f'{filter_direktorija_GB}/base_GB.csv', delimiter=';', engine='python')
        ...
        for week in week_number:
            ...
            # AT, CH DE IN LT
            df_GB = df1.loc[df1['Owner Country Code'] == 'GB']
            rows_GB, columns = df_GB.shape
            total_GB.append(rows_GB)
            
            df_IE = df1.loc[df1['Owner Country Code'] == 'IE']
            rows_IE, columns = df_IE.shape
            total_IE.append(rows_IE)
            ...
            #LT_DE, LT_CH, LT_AT
            IE_total = df_lt.loc[df_lt['Queue'].str.contains("TH_GB")]
            rows, columns = IE_total.shape
            total_LT_GB.append(rows)

            IE_total = df_lt.loc[df_lt['Queue'].str.contains("TH_IE")]
            rows, columns = IE_total.shape
            total_LT_IE.append(rows)
            ...
            empty_level = rows_lt - rows1- rows2 -rows3 - rows4 
            know0.append(empty_level)
            
            
   

        base_by_week = pd.read_csv(f'{final_direktorija_GB}/base_by_week.csv', sep=',')
        ...
        data = {'Year':years,  'Month':month_list, 'Week': week_number,'request number': total,'request number in LT': total_LT, 
                'request number in GB':total_GB , 'request number in IE':total_IE , 'request number in IN':total_IN,
                'IMP request number': total_LT_IMP, 'SMART request number':totoal_LT_SMART, 'STRAT request number': total_LT_STRAT,
                'EMPTY request number': total_LT_EMPTY, 'LT request number in GB':total_LT_GB,
                'LT request number in IE':total_LT_IE, 'LT CTS': total_LT_CTS,
        ...
        last_four_weeks.to_csv(f'{final_direktorija_GB}/base_by_last_weeks.csv', sep=',', index=False)

        final_data_by_week.to_csv(f'{final_direktorija_GB}/base_by_week.csv', sep=',', index=False)
